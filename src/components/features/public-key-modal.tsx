...
export function PublicKeyModal() {
  ...
  const [draftReimbursementFee, setDraftReimbursementFee] = useState<
    number | ""
  >(reimbursementFee === 0 ? 1000 : reimbursementFee);
  ...
  useEffect(() => {
    ...
    setDraftProviderRut(providerRut);
  ...
  const isMetaProvider = isCnplMetaProviderPublicKey(draftKey);
  const isKeyMissing = draftKey.trim() === "";
  const isSecretMissing = isCnpl && draftClientSecret.trim() === "";
  const isProviderRutMissing =
    isCnpl && isMetaProvider && draftProviderRut.trim() === "";
  const canSave = !isKeyMissing && !isSecretMissing && !isProviderRutMissing;

  const handleSave = () => {
    setPublicKey(draftKey);
    setClientSecret(draftClientSecret);
    setProviderRut(isMetaProvider ? draftProviderRut.trim() : "");
    const preset =
    ...
    setReimbursementFee(
      draftReimbursementMode === "free_trial"
        ? 0
        : draftReimbursementFee === ""
          ? 0
          : draftReimbursementFee,
    );
              ...
              <label className="flex cursor-pointer items-center gap-3 rounded-lg border border-gray-200 px-3 py-2.5 transition hover:bg-gray-50 has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-gray-400">
                    Registro de paciente para reembolso
                  </span>
                </div>
              </label>
              <label className="flex cursor-pointer items-center gap-3 rounded-lg border border-gray-200 px-3 py-2.5 transition hover:bg-gray-50 has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-gray-400">
                    El paciente paga 30% de la boleta y el resto con el
                    reembolso
                  </span>
                </div>
              </label>
            ...
            <section className="flex flex-col gap-1.5">
              ...
              {isSecretMissing && (
                <p className="text-xs text-red-500">
                  Este campo es obligatorio para el flujo CNPL.
                </p>
              )}
            </section>
          )}

          {isCnpl && isMetaProvider && (
            <section className="flex flex-col gap-1.5">
              <label
                htmlFor="provider-rut-input"
                className="flex items-center gap-1.5 text-sm font-medium text-gray-700"
              >
                Rut centro médico
                <InfoTooltip text="Se envía como provider_rut al crear la orden CNPL para un meta provider." />
              </label>
              <input
                id="provider-rut-input"
                value={draftProviderRut}
                onChange={(e) => setDraftProviderRut(e.target.value)}
                placeholder="76.123.456-7"
                className={`w-full rounded-lg border px-3 py-2 text-sm text-gray-900 placeholder-gray-400 outline-none transition focus:ring-2 ${
                  isProviderRutMissing
                    ? "border-red-400 focus:border-red-500 focus:ring-red-500/20"
                    : "border-gray-200 focus:border-blue-500 focus:ring-blue-500/20"
                }`}
              />
              {isProviderRutMissing ? (
                <p className="text-xs text-red-500">
                  Este campo es obligatorio para crear la orden CNPL de un meta
                  provider.
                </p>
              ) : (
                <p className="text-xs text-gray-400">
                  Sólo se usa al crear la orden de pago cuando la public key
                  corresponde a un meta provider.
                </p>
              )}
            </section>
          )}
